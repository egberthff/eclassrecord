...
<div class="row">
    <div class="btn_attandance col-md-4" style="padding-top: 10px;padding-left: 25px;">
    @isset($faculty)
...
<div class="container">
            ...
            <form method="POST" action="{{route('attendance.store')}}" enctype="multipart/form-data"
                    ...
                    <div class="col-md-12">
                      ...
                      ?>
                            ...
                            <select class="form-control select2" name="curriculum_subject_id" id="company"
                                ...
                                style="font-weight: bold;" onchange="handleChange(this.value)" @isset($model) enabled
                                @else disabled @endisset>
                                @isset($subjs)
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="company" class="form-control" name="items_total" aria-required="true"
                        aria-invalid="false" data-validation="required" min="1" @isset($model) enabled @else disabled
                        @endisset>
                </div>
                <br>

                <div class="form-group">
                    <input type="submit" id="country" class="form-control btn btn-success" value="Submit" @isset($subjs)
                        enabled @else disabled @endisset @isset($model) enabled @else disabled @endisset>
                </div>
            </form>
        </div>
        <div class="col-md-8">
                                    ...
                                    <tr class="tr-shadow" style="font-size: 12px; padding: 5px;">
                                        ...
                                        <td>
                                            <div class="table-data-feature">
                                                @if($element->status == 1)
                                                <a href="{{route('attendance.edit', $element->id)}}" style="margin-top: 10px;" class="item" data-toggle="tooltip"
                                                    data-placement="top" title="EDIT">
                                                    <i class="fs-5 bi bi-pencil-fill" style="color: blue;"></i>
                                                </a>
                                                <a href="{{route('attendance_add_score', $element->id)}}"
                                                    style="margin-top: 10px;" class="item" data-toggle="tooltip"
                                                    data-placement="top" title="UPLOAD SCORES">
                                                    <i class="fs-5 bi bi-upload" style="color: green;"></i>
                                                </a>
                                                <b style="color: green;">
                                                    <a href="{{route('attendance.destroy', $element->id)}}" style="margin-top: 10px;" class="item btnRem"
                                                ...
                                                @else
                                                <a href="{{route('attendance.destroy', $element->id)}}" style="margin-top: 10px;" class="item btnAct"
                                                    data-toggle="tooltip" data-placement="top"
    ...
    $(document).ready(function() {
        ...
        swal(
            'Activated',
            'Attendance has been removed from archive successfuly.',
            'success'
        )
        @elseif(Session::has('Archived'))
        swal(
            'Deleted',
            'Attendance has been added to archive.',
            'success'
        )
