  ...
  <div class="container">
    <div class="card">
      <div class="card-header">
        <b>{{$collection->curriculum_subject->subject_code}} [{{$collection->curriculum_subject->course->code}} {{$collection->curriculum_subject->year}} - {{$collection->curriculum_subject->semester}}]  &nbsp&nbsp&nbsp&nbsp&nbsp Section : {{$collection->section}}</b>
      </div>
      <div class="card-body">
        <div class="row">
            <div class="col-md-3"></div>
            <div class="col-md-6" style="text-align: center;margin: 0px;">
                <p style="margin: 0px;font-size: 20px;font-weight: bold;">{{$collection->name}} - Attendance</p>
            ...
            <tbody>
                @isset($students)
                    @foreach($students as $element)
                        <?php 
                            $score = App\AttendanceScore::where('attendance_id', $collection->id)->where('id_no', $element->id_no)->first();
                        ?>
                    <tr>
                        <td>{{$element->id}}</td>
                        <td>{{$element->lastname}}, {{$element->firstname}} {{$element->middlename}}</td>
                        <td><input type="number" @isset($score) value="{{$score->score}}" @else value="0" @endisset onchange="changeScore(this, {{$element->id_no}}, {{$collection->id}})" max="{{$collection->items_total}}" style="text-align: center;"></td>
                    </tr>
                    @endforeach
                @endisset
            </tbody>
