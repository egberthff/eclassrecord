...
<div class="row">
    ...
    <div class="col-md-2">
        <a href="{{route('gradesprint')}}" class="btn btn-info" target="_blank" style="margin-top: 20px;"><i class="bi bi-printer"></i> PRINT</a>
    </div>
    ...
    <div class="card">
        <div class="container-fluid row pull-center" style="text-align: right;padding-top: 10px;padding-bottom: 10px;">
            <div class="col-md-12" style="text-align: center;">
                <img src="{{URL::to('header.png')}}" style="width: 80%;height:170px;" alt="">
            </div>

        </div>
        <div class="container-fluid">
            <p class="cambria margin-0rem" style="text-align: center;font-weight: bold;font-size: 20px;padding-top: 10px;border-top: 3px solid;">GRADING SHEET</p>
            <p class="cambria margin-0rem" style="text-align: center;padding-bottom: 10px;border-bottom: 3px solid;"><b>{{$sy->name}}</b>, Term: <b>{{$collection->semester}}</b></p>

            <div class="row">
                <div class="col-md-3">Section: <b>@isset($collection->course){{$collection->course->code}} @endisset @isset($collection->year) {{$collection->year}}@endisset @isset($section_activ){{$section_activ}} @endisset</b></div>
            ...
            <br>
            <div class="row">
                <div class="col-md-3">Subject Code: <b>@isset($collection->subject_code){{$collection->subject_code}} @endisset</b></div>
                <div class="col-md-3">Subject Description: <b>@isset($collection->subject_desc){{$collection->subject_desc}} @endisset</b></div>
                <div class="col-md-3"></div>
                <div class="col-md-3">Units: <b>@isset($collection->total_units){{$collection->total_units}} @endisset</b></div>
            </div>
            <br>
                    ...
                    @isset($students)
                        ...
                        @foreach($students as $stud)
                            ...
                            <tr>
                                ...
                                @if(Auth::user()->prelim == 1)
                                @if($grade > 3)
                                    <td style="text-align: right;font-weight: bold;color: red;">{{number_format($grade ?? 0, 1)}}</td>
                                @else
                                    <td style="text-align: right;font-weight: bold;color: green;">{{number_format($grade ?? 0, 1)}}</td>  
                                @endif
                                @endif
                                @if(Auth::user()->midterm == 1)
                                @if($grade_mid > 3)
                                    <td style="text-align: right;font-weight: bold;color: red;">{{number_format($grade_mid ?? 0, 1)}}</td>
                                @else
                                    <td style="text-align: right;font-weight: bold;color: green;">{{number_format($grade_mid ?? 0, 1)}}</td>  
                                @endif
                                @endif
                                @if(Auth::user()->prefi == 1)
                                @if($grade_prefi > 3)
                                    <td style="text-align: right;font-weight: bold;color: red;">{{number_format($grade_prefi ?? 0, 1)}}</td>
                                @else
                                    <td style="text-align: right;font-weight: bold;color: green;">{{number_format($grade_prefi ?? 0, 1)}}</td>  
                                @endif
                                @endif
                                @if(Auth::user()->final == 1)
                                @if($grade_finals > 3)
                                    <td style="text-align: right;font-weight: bold;color: red;">{{number_format($grade_finals ?? 0, 1)}}</td>
                                @else
                                    <td style="text-align: right;font-weight: bold;color: green;">{{number_format($grade_finals ?? 0, 1)}}</td>  
                                @endif
                                @endif
                                @if($gwa > 3)
                                    <td style="text-align: right;font-weight: bold;color: red;">{{number_format($gwa ?? 0, 1)}}</td>
                                @else
                                    <td style="text-align: right;font-weight: bold;color: green;">{{number_format($gwa ?? 0, 1)}}</td>  
                                @endif
                                <?php 
                                ...
                                @if($gwa > 3)
                                    ...
                                    <select name="re_exam" id="{{$faculty->curriculum_subject_id}}" class="{{$stud->id_no}}" onchange="handleChangeReExam(this)">
                                        <option value="5.0" @isset($re_exam) @if($re_exam->grade == '5.0') selected @endif @endisset>5.0</option>
                                        <option value="3.0" @isset($re_exam) @if($re_exam->grade == '3.0') selected @endif @endisset>3.0</option>
                                    </select></td>
                                    @isset($re_exam) 
                                        @if($re_exam->grade == '3.0')
                                        <td class="status{{$stud->id_no}}" style="text-align: left;font-weight: bold;color: black;"></td> 
                                        @else 
                                        <td class="status{{$stud->id_no}}" style="text-align: left;font-weight: bold;color: red;">Failed</td>
                                        @endif 
                                    @else
                                        <td class="status{{$stud->id_no}}" style="text-align: left;font-weight: bold;color: red;">Failed</td>
                                    @endisset
...
@section('scripts')
  <script>
      ...
      function handleChangeReExam(e) {
        // Check if a value is selected
        var selectedValue = e.value;
        var curr_subj_id = e.id;
        var id_no_ = e.className;
        ...
        $.ajax({
            url: '/handlechangerexam', 
            type: 'POST',
            data: {
                value: selectedValue,
                id_no: id_no_, 
                curriculum_subject_id: curr_subj_id,
                _token: '{{ csrf_token() }}' 
            },
            success: function(response) {
                const tdElement = document.querySelector('.status'+id_no_);
                if (tdElement) {
