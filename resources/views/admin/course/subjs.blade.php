  ...
  <div class="container">
    <div class="card">
      <div class="card-header">
        <b>{{$collection->name}}</b>
        <a href="{{route('courseaddsubjs', $collection->id)}}" class="btn btn-sm btn-success" style="float: right;margin-bottom: 5px;"><i class="bi bi-plus"></i> Add / Update</a>
      </div>
      <div class="card-body">
        <div class="row">
            <div class="col-md-3"></div>
            <div class="col-md-6" style="text-align: center;margin: 0px;">
                <p style="margin: 0px;font-size: 20px;font-weight: bold;">CURRICULUM CHECKLIST</p>
                <p style="margin: 0px;">{{$collection->course}}</p>
                <p style="margin: 0px;">Revised Curriculum Academic Year 2023 – 2024</p>
                <p style="margin: 0px;">Approved: BOR Resolution No. 06-29-139 s. 2023</p>
            </div>
            <div class="col-md-3"></div>
        </div>
        
      <a href="{{route('coursedeleteallsubs', $collection->id)}}" class="btn btn-sm btn-danger" style="float: right;margin-bottom: 5px;margin-left: 50px;"><i class="bi bi-trash"></i> Delete All</a>
        <?php 
          $groupedSubjects = [];
              ...
              <table class="table table-borderless" style="width: 100%;">
                  <thead>
                      <tr>
                          ...
                          <th width="5%">Lec Hours</th>
                          <th width="5%">Lab Hours</th>
                          <th width="5%">Total Units</th>
                          <th width="10%">Pre Requisites</th>
                          <th width="10%">Co Requisites</th>
                          <th width="5%"></th>
                      </tr>
                  </thead>
                  <tbody>
                    @isset($grpsubj)
                      @foreach($grpsubj as $subj)
                        <tr>
                          <td>{{$subj->subject_code}} @if($subj->faculty_id != null) <i class="fs-5 bi bi-pencil-fill"></i> @endif</td>
                          ...
                          <td>{{$subj->pre_reqs}}</td>
                          <td>{{$subj->co_reqs}}</td>
                          <td>@if($subj->faculty_id == null)<a href="{{route('removesubj', $subj->id)}}" title="REMOVE">
